...
export async function handler(args: z.infer<typeof schema>, extra: RequestHandlerExtra) {
  try {
    ...
    // Format as compact report with notes
    const formattedReport = formatCompactReportWithNotes(database, {
...
// Function to truncate and clean up notes text
function formatNote(note: string | null | undefined): string {
  ...
  return cleaned.substring(0, MAX_NOTE_LENGTH) + '...';
}

// Function to format the database in the compact report format with notes
function formatCompactReportWithNotes(database: any, options: { hideCompleted: boolean, hideRecurringDuplicates: boolean }): string {
...
Notes: contained in "..." after item name\n\n`;
  ...
  function processProject(project: any, level: number): string {
    ...
    const noteText = project.note ? ` "${formatNote(project.note)}"` : '';
    
    let projectOutput = `${indent}P: ${project.name}${flaggedSymbol}${statusInfo}${noteText}\n`;
  ...
  function processTask(task: any, level: number): string {
    ...
    const noteText = task.note ? ` "${formatNote(task.note)}"` : '';
    
    let taskOutput = `${indent}• ${flagSymbol}${task.name}${dateInfo}${durationStr}${tagsStr}${statusStr}${noteText}\n`;
